...
Call the Agent tool once per chunk — ALL in the same response so they run in parallel. Build the message by wrapping the extraction prompt below in task-delegation framing:
...
Use `subagent_type="general-purpose"` for all chunks. After all agents are dispatched, collect results sequentially.
...
Read `.graphify_analysis.json`. For each community key, look at its node labels and write a 2-5 word plain-language name (e.g. "Attention Mechanism", "Training Pipelines", "Data Loading").
...
Replace `LABELS_DICT` with the actual dict you constructed (e.g. `{0: "Attention Mechanism", 1: "Training Pipelines"}`).
Replace INPUT_PATH with the actual path.
...
to_obsidian(G, communities, labels, out)
to_canvas(G, communities, labels, out.parent / 'graph.canvas')
print(f'Obsidian vault written to {out}')
"
```

**HTML generation** (always, unless `--no-viz`):
...
### Step 7 - Cleanup + print report

```bash
rm -f .graphify_detect.json .graphify_extract.json .graphify_analysis.json .graphify_labels.json .graphify_ast.json .graphify_semantic.json
```

Then read and present `graphify-out/GRAPH_REPORT.md` to the user. Highlight:
- God nodes (highest-degree concepts)
- Top 3-5 surprising connections with their "why" explanations
- Suggested questions the graph is uniquely positioned to answer
- Token benchmark (input/output tokens used in extraction)

## After the Graph is Built

Once the graph exists, users can query it directly:

```bash
/graphify query "what connects attention to the optimizer?"
/graphify query "what connects attention to the optimizer?" --dfs   # DFS traversal
/graphify query "what is CfgNode?" --budget 500
/graphify path "DigestAuth" "Response"       # shortest path
/graphify explain "SwinTransformer"           # node explanation
```

These commands traverse `graphify-out/graph.json` hop by hop, trace exact paths between nodes, and surface edge-level detail (relation type, confidence score, source location).

## Always-On Mode

After building a graph, run this once:

```bash
graphify install --platform qwen-code
```

This writes to `AGENTS.md` in your project root, telling Qwen Code to read `graphify-out/GRAPH_REPORT.md` before answering architecture questions. Qwen Code reads AGENTS.md automatically every session — no hook needed.

## Incremental Updates

For code changes only (no docs/papers/images):

```bash
/graphify <path> --update          # re-extract changed files, merge into existing graph
/graphify <path> --watch           # auto-sync as files change
```

The SHA256 cache means re-runs only re-process changed files. Code updates use AST only — no LLM needed.

For full re-extraction including docs:

```bash
/graphify <path> --update          # same command, detects all file types
```
...
```bash
/graphify <path> --neo4j           # generates graphify-out/cypher.txt
/graphify <path> --neo4j-push bolt://localhost:7687   # push to running Neo4j
```

## MCP Server

Expose the graph as an MCP stdio server for repeated queries:

```bash
python -m graphify.serve graphify-out/graph.json
```

Gives structured graph access for tools like `query_graph`, `get_node`, `get_neighbors`, and `shortest_path`.

## Platform Notes for Qwen Code

- **Agent tool**: Use `subagent_type="general-purpose"` for all semantic extraction subagents
- **Parallel dispatch**: Launch all subagents in a single response for maximum parallelism
- **AGENTS.md**: Qwen Code reads this file automatically — no PreToolUse hooks needed
- **Skills**: Installed to `.qwen/skills/graphify/SKILL.md` and invoked via `/graphify`
